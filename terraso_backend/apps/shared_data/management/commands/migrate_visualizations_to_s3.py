...
class Command(BaseCommand):
    help = (
        "Migrates existing visualization configs from Mapbox tilesets/"
        "inline GeoJSON to S3 GeoJSON"
    )
    ...
    def handle(self, *args, **options):
        ...
        for vc in vcs:
            try:
                ...
                migrated_vc_count += 1
                self.stdout.write(
                    f"MIGRATED VC {vc.id} ({vc.title}): key={path}"
                )
            except Exception as e:
                self.stdout.write(
                    self.style.ERROR(
                        f"ERROR VC {vc.id} ({vc.title}): {e}"
                    )
                )
                logger.exception(
                    "Migration error", extra={"vc_id": vc.id}
                )
